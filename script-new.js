...
async function loadStudyModeData() {
    if (appState.selectedStudyMode === 'spaced') {
        ...
        const { data, error } = await FlashcardManager.getFlashcardSets();
        ...
        const { data: flashcards, error: cardsError } = await FlashcardManager.getFlashcardsInSet(firstSet.id);
...
async function stopStudyMode() {
    ...
    elements.studyRating.classList.add('hidden');
